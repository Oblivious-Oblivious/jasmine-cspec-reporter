...
const custom_reporter = {
    ...
    specDone: function (result) {
        ...
        if (result.status === "passed") {
            ...
            console.log(
                this.spacing + "✓".green() + ` it ${result.description}`,
            );
        } else if (result.status === "pending") {
            this.xit_counter++;
            console.log(
                this.spacing + `- xit ${result.description} (skipped)`.gray(),
            );
        } else {
            ...
            result.failedExpectations.forEach(e => {
                ...
                if (e.actual === "")
                    console.log(
                        this.spacing +
                            "        |> " +
                            e.message.replace("Failed: ", "").red(),
                    );
                else console.log(this.spacing + "        |> " + e.message);
            });
            console.log("");
        }
    ...
    jasmineDone: function (_) {
        ...
        console.log(
            `✗ ${
                this.it_counter - this.positive_it_counter - this.xit_counter
            } failing`.red(),
        );
        console.log(`- ${this.xit_counter} skipped`.gray());
        if (this.total_time > 1000)
            console.log(
                `★ Finished in ${(this.total_time / 1000.0).toFixed(
                    5,
                )} seconds`.cyan(),
            );
        else if (this.total_time > 60000)
            console.log(
                `★ Finished in ${(this.total_time / 60000.0).toFixed(
                    5,
                )} minutes`.cyan(),
            );
        else
            console.log(
                `★ Finished in ${this.total_time.toFixed(5)} ms`.cyan(),
            );
    },
};
